                    ...
                    @foreach ($surveys as $key => $survey)
                        <tr class="bg-white border-b hover:bg-gray-50">
                            ...
                            <td class="py-4 px-6 text-center">
                                {{ $survey->count_question }}
                            </td>
                            <td class="py-4 px-6 text-center">
                                {{ $survey->participant->count() }}
                            </td>
                            <td class="px-6 py-4 content-center w-40 flex justify-center gap-2 items-center">
                                <div class="flex">
                                    <a href="{{ route('admin.answer.detail', ['survey' => $survey]) }}"
                                        class="text-white px-2 rounded-xl py-2 flex justify-center items-center gap-2 hover:bg-primary hover:bg-opacity-90 bg-primary">
                                ...
                                <div class="flex">
                                    <a href="{{ route('admin.answer.chart', ['survey' => $survey]) }}"
                                        class="text-white px-2 rounded-xl py-2 flex justify-center items-center gap-2 hover:bg-primary hover:bg-opacity-90 bg-primary">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                            stroke-width="1.5" stroke="currentColor" class="size-5">
                                            <path stroke-linecap="round" stroke-linejoin="round"
